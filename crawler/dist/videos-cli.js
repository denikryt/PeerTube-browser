...
program
    ...
    .option("--new-videos", "Skip videos that already exist in the database (by video_id + instance_domain)", false)
    .option("--stop-after-full-pages <number>", "Stop after N consecutive pages where all videos already exist (requires --new-videos; default: 0)", "0")
    .option("--sort <value>", "Sort value passed to /video-channels/:name/videos (example: -publishedAt)", "-publishedAt")
    .option("--max-instances <number>", "Limit number of instances to process (0 = no limit)", "0")
    .option("--max-channels <number>", "Limit number of channels to process (0 = no limit)", "0")
    .option("--max-videos-pages <number>", "Limit number of pages fetched per channel (0 = no limit)", "0")
    .option("--tags", "Tags-only mode: fetch per-video tags via /api/v1/videos/:uuid for videos with tags_json NULL or []", false)
    .option("--update-tags", "Tags-only mode: refresh tags via /api/v1/videos/:uuid for videos with existing tags_json", false)
    .option("--comments", "Comments-only mode: fetch per-video comments via /api/v1/videos/:uuid", false)
    .option("--host-delay <ms>", "Delay between requests per host in tags/comments mode", "200")
