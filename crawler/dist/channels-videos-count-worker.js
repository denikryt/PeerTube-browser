...
export async function crawlChannelVideosCount(options) {
    ...
    updateStatus(`[channels-videos] instances=${hosts.length} concurrency=${workerCount} resume=${options.resume} errorsOnly=${options.errorsOnly}`);
    updateProgress(progress);
    updateStatus("[channels-videos] idle");
    const queue = hosts.slice();
    const workers = Array.from({ length: workerCount }, () => workerLoop(queue, store, options, progress));
...
async function processInstance(host, store, options, progress) {
    ...
    try {
        const { total, updated } = await updateVideosCountForInstance(normalizedHost, store, options, progress);
        updateStatus(`[channels-videos] done ${normalizedHost} updated=${updated} total=${total}`);
    }
    catch (error) {
...
async function updateVideosCountForInstance(host, store, options, progress) {
    ...
    await mapWithConcurrency(channels, CHANNEL_CONCURRENCY, async (channel) => {
        ...
        if (channel.videos_count !== null) {
            updateStatus(`[channels-videos] skip ${host}/${channel.channel_name} videos_count=${channel.videos_count}`);
            updateProgress(progress);
            return;
        }
        if (options.errorsOnly && channel.last_error_source !== "videos_count") {
            updateStatus(`[channels-videos] skip ${host}/${channel.channel_name} no_error`);
            updateProgress(progress);
            return;
        }
        if (options.resume && channel.last_error_source === "videos_count") {
            updateStatus(`[channels-videos] skip ${host}/${channel.channel_name} error=${channel.last_error ?? "unknown"}`);
            updateProgress(progress);
            return;
        }
        const hadError = channel.last_error_source === "videos_count";
        const videosCount = await fetchChannelVideosCount(host, channel.channel_name, options, (message) => updateStatus(message));
        ...
        updateStatus(`[channels-videos] done ${host}/${channel.channel_name} videos_count=${videosCount.total}`);
        updateProgress(progress);
    });
