...
10. **47** then **16l** then **39** then **44** then **40** (smoke tests + cache runtime safety + similarity shadow swap + zero-downtime deploy)  
   Lock smoke coverage for installer/runtime interaction contracts, then add background/atomic cache refresh primitives, then startup no-downtime hardening, then similarity-cache shadow cutover, then blue/green nginx switch automation.
...
- **Block G: Runtime reliability and operations**
  ...
  - Scope: smoke coverage for install/runtime interaction contracts, safe cache refresh/swap runtime behavior (random + similarity), and automated blue/green nginx cutover.
