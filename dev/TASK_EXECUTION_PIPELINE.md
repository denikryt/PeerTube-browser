...
10. **51** then **47** then **16l** then **39** then **44** then **40** (uninstall symmetry + smoke tests + cache runtime safety + similarity shadow swap + zero-downtime deploy)  
   Land symmetric uninstall contracts on top of the existing installer architecture, then lock smoke coverage for installer/runtime interaction contracts, then add background/atomic cache refresh primitives, then startup no-downtime hardening, then similarity-cache shadow cutover, then blue/green nginx switch automation.
...
- **Block G: Runtime reliability and operations**
  - Tasks: **51 -> 47 -> 16l -> 39 -> 44 -> 40**
  - Scope: symmetric uninstall contracts, smoke coverage for install/runtime interaction contracts, safe cache refresh/swap runtime behavior (random + similarity), and automated blue/green nginx cutover.
...
- **51 <-> 47**: installer smoke tests depend on deterministic teardown semantics; reusable uninstall scripts reduce duplicate cleanup logic and flaky residual-state failures.  
  Keep smoke cleanup aligned with uninstaller source of truth.
