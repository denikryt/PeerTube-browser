...
### Expected Behaviour
- The workflow model uses one clear separation of concerns: `Feature` and `Issue` are always GitHub-tracked entities, while local decomposition is represented through `Task` units owned by a feature or issue.
- Repository terminology stops overloading `issue` to mean both an internal implementation stage and an external tracker: internal decomposition uses `Task`, while `Issue` means an entity that is explicitly tracked on GitHub.
- Workflow rules, protocols, CLI behavior, and tracker semantics align around feature-first execution, stable local IDs independent from hierarchy moves (`F1`, `I1`, `M1`), a simplified `Pending -> Draft -> Planned -> Done` status model, optional local task decomposition with no DEV_MAP runtime ownership, and explicit local-vs-remote synchronization behavior without backward-compatibility migration requirements for already-finished plans.

### I1-F18-M1 - Define canonical local planning model and terminology

#### Expected Behaviour
- The repository has one explicit planning model that defines only three primary entity types: `Feature`, `Issue`, and `Task`.
- `Feature` and `Issue` are always GitHub-tracked entities, while `Task` is the canonical local decomposition unit used inside feature and issue plans.
- A feature-level remote issue represents the feature itself, child issues are GitHub-tracked issue entities attached to a feature by owner chain, and standalone issues remain GitHub-tracked entities with no feature owner.
- Runtime statuses are reduced to `Pending`, `Draft`, `Planned`, and `Done`; `Tasked` is removed because local tasks no longer need a dedicated runtime tracker state in `DEV_MAP`.
- Local IDs are stable and hierarchy-independent (`F1`, `I1`, `M1`); milestone assignment and feature ownership are stored as separate fields rather than encoded into the identifier string.

#### Dependencies
- file: `.agents/rules/feature-planning.md` | reason: hard policy currently binds feature decomposition and execution readiness to `Issue` semantics and GitHub mappings.
- file: `.agents/protocols/feature-planning-protocol.md` | reason: planning contract currently defines feature-to-issue decomposition as the mandatory internal model.
- file: `AGENTS.md` | reason: top-level repository guidance must use the same terms and command expectations as the canonical workflow docs.
- file: `dev/FEATURE_PLANS.md` | reason: stored plans must reflect the final terminology and feature-level decomposition model.
- file: `dev/map/DEV_MAP.json` | reason: tracker nodes and status-bearing entities must reflect the clarified distinction between GitHub-tracked entities and local task decomposition.

#### Decomposition
1. Define the canonical vocabulary and ownership boundaries.
   - Identify which current terms are overloaded across planning, tracking, and GitHub publication.
   - Specify the target meaning for `Feature`, `Issue`, and local `Task` units, including feature-owned issues and standalone issues.
   - Validation: grep audit shows no unresolved ambiguity in the rewritten canonical docs for `Feature`, `Issue`, `Task`, `feature_id`, and `milestone_id`.
   - Output: one repository-wide terminology contract that can be referenced without ambiguity.
2. Translate the terminology contract into planning rules.
   - Rewrite planning guidance so local decomposition uses `Feature -> Task` and `Issue -> Task`, with at least one task required for every planned feature and every planned issue.
   - State that `Issue` is always GitHub-tracked by definition, while `Task` remains a local planning/execution artifact that is not runtime-tracked in `DEV_MAP`.
   - Validation: protocol/rule docs express the same entity/status model and contain no target-state references to `Tasked`, `plan tasks for`, or task runtime ownership in `DEV_MAP`.
   - Output: protocol/rule wording that gives the agent one stable planning model to follow.
3. Align stored planning artifacts with the terminology shift.
   - Update feature-plan expectations so future sections in `dev/FEATURE_PLANS.md` reflect the clarified local planning model and GitHub-tracked entity vocabulary.
   - Replace the old status contract with `Pending -> Draft -> Planned -> Done` across plan/runtime guidance and remove `Tasked` references from the new model.
   - Replace hierarchy-encoded IDs with stable local IDs and explicitly state that migration/version moves do not change entity identity.
   - Explicitly state that no backward-compatibility migration is required for already-existing planned/finished features; this feature governs only the new contract going forward.
   - Validation: `python3 dev/workflow feature plan-lint --id F18-M1` remains valid after the terminology/status rewrite.
   - Output: planning artifacts and rules that refer to the same conceptual model without legacy-migration scope.

#### Issue/Task Decomposition Assessment
- Expected split: 3 implementation tasks
  1. canonical terminology/type model for Feature/Issue/Task roles
  2. planning-rule rewrite with explicit GitHub-tracked issue semantics and simplified statuses
  3. plan/tracker contract alignment without legacy migration work

### I2-F18-M1 - Refactor planning and execution workflows around feature-first local decomposition

#### Expected Behaviour
- Workflow procedures and CLI-facing docs treat `Feature` and `Issue` as the primary planning/execution scopes, while `Task` is the shared internal decomposition unit for both.
- The command surface keeps separate `plan feature`, `plan issue`, `execute feature`, and `execute issue` commands, but removes `plan tasks for ...` and `execute task` from the new model because task decomposition is not a separate tracked execution phase.
- The repository replaces ambiguous `confirm` wording with explicit `done feature <id>` / `done issue <id>` commands that set local status to `Done`, and optionally update remote state only when `--remote` is provided.

#### Dependencies
- file: `.agents/workflows/plan-feature.md` | reason: feature planning currently mandates decomposition into issue nodes and uses that as the primary planning path.
- file: `.agents/workflows/plan-issue.md` | reason: single-issue planning workflow must be reconciled with the new feature-first local decomposition model.
- file: `.agents/workflows/execute-feature.md` | reason: feature execution currently hard-gates on mapped child issues instead of local readiness.
- file: `.agents/workflows/execute-features.md` | reason: multi-feature wrapper must remain a thin wrapper over the revised feature execution contract.
- file: `.agents/workflows/execute-issue.md` | reason: issue execution semantics must stay coherent for GitHub-tracked child issues and standalone issues.
- file: `.agents/workflows/confirm.md` | reason: confirmation semantics must either be narrowed to a final status transition or replaced with a simpler done/close flow.
- file: `.agents/workflows/reject.md` | reason: reject and done flows must stay complementary once confirm is replaced.
- file: `.agents/protocols/task-execution-protocol.md` | reason: execution readiness and branch/materialization standards must match the redesigned workflow semantics.
- file: `dev/workflow_lib/feature_commands.py` | reason: runtime command behavior and validation must eventually match the rewritten workflow contracts.

#### Decomposition
1. Stage A: Lock the new data model and lifecycle contract before touching command behavior.
   - Specify the final entity model (`Feature`, `Issue`, `Task`), stable IDs (`F1`, `I1`, `M1`), required ownership fields (`feature_id`, `milestone_id`), and the runtime status chain `Pending -> Draft -> Planned -> Done`.
   - Remove the target-state use of `Tasked` and task runtime ownership in `DEV_MAP` from the contract before rewriting command semantics.
   - Validation: canonical docs and schemas agree on IDs, fields, and statuses; repo grep shows no target-state references to `Tasked`.
   - Output: one locked data/lifecycle contract that the remaining stages can implement against.
2. Stage B: Rewrite the CLI contract around the locked model.
   - Keep `plan feature`, `plan issue`, `execute feature`, and `execute issue`; remove `plan tasks for ...` and `execute task` from the target model.
   - Replace `confirm` with `done feature <id>` / `done issue <id>` and define `--remote` as the only trigger for remote close/project-status sync.
   - Define `move issue <issue_id> --feature <feature_id>` as the ownership-move command for attaching standalone issues to a feature.
   - Validation: CLI help/output contract and command-routing docs expose the new commands only; grep shows no stale target-state references to removed commands in canonical procedure docs.
   - Output: one explicit CLI contract for planning, execution, move, done, and remote sync.
3. Stage C: Rewrite planning and execution procedures around direct task-in-plan authoring.
   - Update `plan feature` and `plan issue` workflows so they write the full local task decomposition directly into `FEATURE_PLANS.md`, without a separate decomposition-sync phase.
   - Update `execute feature` and `execute issue` workflows so readiness checks depend on the new statuses, required GitHub tracking for feature/issue entities, and non-gating local tasks.
   - Validation: workflow docs, rules, and protocols read coherently end-to-end and no longer prescribe removed intermediate phases.
   - Output: planning/execution procedures that can be followed deterministically from local context alone.
4. Stage D: Rewrite completion semantics and edge-case ownership behavior.
   - Define `done` as a narrow completion command for feature/issue entities: local `Done` always, remote completion only under `--remote`.
   - Define how `done issue` behaves for standalone issues, child issues, and the feature-level issue, and keep `reject` behavior coherent with the simplified lifecycle.
   - Validation: completion/rejection docs and runtime behavior describe the same state transitions and remote side effects.
   - Output: one deterministic completion contract for feature and issue entities.
5. Stage E: Apply the runtime rewrite in code and trackers as one coordinated change set.
   - Update command handlers, validation logic, status checks, `DEV_MAP` shape/field handling, and publish/materialize behavior to match Stages A-D.
   - Keep local tasks out of runtime tracking, preserve stable IDs during ownership/milestone moves, and ensure imported GitHub issues can be moved into a feature through the new ownership command.
   - Validation: CLI smoke/validation flows pass for create/plan/execute/done/materialize/move paths under the new model.
   - Output: runtime implementation that matches the locked contract instead of a hybrid transitional state.
6. Stage F: Run one final consistency and artifact-cleanup pass.
   - Audit canonical docs, workflows, runtime code, and trackers for removed concepts (`Tasked`, `plan tasks for`, `execute task`, `confirm` as target-state command).
   - Remove stale references, stale helper paths, and obsolete compatibility wording so the repository reflects one model only.
   - Validation: targeted grep audit is clean, feature plan lint stays valid, and workflow/CLI validation passes without leftover artifacts from the old model.
   - Output: one artifact-clean repository state with no mixed-model residue.

#### Issue/Task Decomposition Assessment
- Expected split: 6 implementation stages
  1. Stage A: data model and lifecycle contract lock
  2. Stage B: CLI contract rewrite
  3. Stage C: planning/execution workflow rewrite
  4. Stage D: completion and ownership-edge semantics
  5. Stage E: runtime/tracker implementation rewrite
  6. Stage F: consistency audit and artifact cleanup

### I3-F18-M1 - Decouple GitHub materialization from local execution readiness

#### Expected Behaviour
- GitHub publication remains the mechanism for synchronizing local entities with remote trackers, and both features and issues are GitHub-tracked by definition before execution.
- Materialization/publish workflows create or sync the feature issue explicitly, create or sync child and standalone issues explicitly, and may support `sync all children` only for already-existing child issues.
- Rules and command behavior make it clear that tasks stay local and off the DEV_MAP runtime path, while imported GitHub issues enter the system as standalone issues that may later be attached to a feature through explicit CLI actions.
- The new model does not preserve move history for milestone/version migrations; stable local IDs remain unchanged while current placement fields are updated in-place.

#### Dependencies
- file: `.agents/rules/feature-planning.md` | reason: current hard policy makes mapped GitHub issue metadata a blocking prerequisite for feature/issue/task execution.
- file: `.agents/workflows/materialize-feature.md` | reason: materialization flow must be reframed as remote publication/synchronization rather than implicit execution authorization.
- file: `.agents/workflows/execute-feature.md` | reason: execute-feature currently hard-stops when child issue GitHub mapping is missing.
- file: `.agents/workflows/execute-issue.md` | reason: issue execution and imported-issue handling must remain coherent with the new publication semantics.
- file: `.agents/workflows/reject.md` | reason: reject/cleanup behavior may also need narrowing once local task tracking and confirm semantics are simplified.
- file: `.agents/protocols/task-execution-protocol.md` | reason: execution-readiness, completion, and branch/materialization guidance must distinguish local authority from remote publishing.
- file: `dev/map/DEV_MAP.json` | reason: tracker semantics for GitHub linkage must stay explicit for features and issues, while local tasks can be removed from DEV_MAP-owned runtime tracking.

#### Decomposition
1. Stage A dependency: define the local-vs-remote authority boundary.
   - State that feature execution requires a materialized feature issue and issue execution requires a materialized issue, because both entities are GitHub-tracked by definition.
   - Define how standalone issues and GitHub-imported issues enter local tracking and how they can later attach to a feature without changing the core type model.
   - Keep migration handling simple: moving a feature or issue between milestones/versions updates current fields only and does not create migration history artifacts.
   - Validation: command/docs/runtime all enforce the same publication prerequisites for feature and issue execution.
   - Output: a precise rule set for required feature/issue publication and local task ownership.
2. Stage E dependency: rewrite publication/materialization workflow semantics.
   - Update materialization guidance so it describes remote creation/sync inputs, outputs, and failure paths for feature issues and child/standalone issues only.
   - Keep optional `sync all children` support for already-materialized child issues, but prevent it from silently creating new issues or treating local tasks as GitHub entities.
   - Validation: materialize/publish smoke scenarios show no hidden child creation and no task-to-GitHub leakage.
   - Output: a remote-publication workflow contract that supports explicit issue publication plus safe child sync.
3. Stage E/F dependency: align tracker/runtime semantics and validation.
   - Ensure `DEV_MAP` GitHub fields and runtime validation logic reflect required publication state for features/issues, while local task decomposition no longer needs full runtime tracking in `DEV_MAP`.
   - Define deterministic behavior for materialized feature execution, materialized issue execution, imported remote issues, and the simplified `Pending -> Draft -> Planned -> Done` lifecycle.
   - Validation: tracking/repo validation passes and canonical docs match the runtime behavior without residual old-state assumptions.
   - Output: tracker semantics and runtime expectations that separate GitHub-tracked entities from local tasks cleanly.

#### Issue/Task Decomposition Assessment
- Expected split: 3 implementation tasks
  1. required feature/issue publication contract
  2. materialize/publish rewrite with safe `sync all children`
  3. tracker/runtime semantics for materialized entities, local tasks, simplified statuses, and stable IDs
