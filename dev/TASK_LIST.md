...
### 54) Structured JSON logs with mode tags and runtime-readable filters
**Problem:** current plain-text logs require fragile regex filtering in `journalctl`, and switching app log profiles requires process restart.

**Solution option:** standardize Engine logs as structured JSON events with explicit mode tags (`modes`) and add stable operator tooling for mode-based viewing without server restarts.

#### **Solution details:**
- **JSON log contract**:
  - emit one JSON object per log event (`ts`, `level`, `event`, `message`, `request_id`, context fields),
  - include `modes` array per event (for example: `["focused", "verbose"]` or `["focused"]`),
  - keep stable event names for recommendation/similarity request flow.
- **Mode semantics**:
  - `modes` drives read-time filtering only (no logic branching in request handling),
  - one event may belong to multiple modes,
  - unsupported/empty mode filtering must fail safely (no crashes; clear output behavior).
- **Runtime operator flow (no restart)**:
  - keep Engine running in one logging format/config,
  - add a dedicated log-view script (for example `dev/watch-engine-logs.sh`) that filters by `modes` via `journalctl + jq`,
  - support at least `focused` and `verbose` views from the same live stream.
- **Backward compatibility and rollout**:
  - keep warning/error visibility in all views,
  - preserve request correlation fields introduced by request lifecycle tasks.
- **Validation / tests**:
  - unit tests for JSON formatter/filter helpers,
  - smoke check that one request emits valid JSON events with expected `modes`,
  - operator check that switching viewer mode changes output without restarting Engine.
