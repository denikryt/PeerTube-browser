...
### 47) Smoke tests for installers/uninstallers and Engine/Client service interaction (with guaranteed cleanup)
**Problem:** current smoke scope is not strict enough to prove that every installer/uninstaller entrypoint works as expected, so wrapper/service-specific/centralized scripts can regress independently without deterministic detection.

**Solution option:** extend smoke coverage to a full installer/uninstaller contract matrix so final implementation guarantees all scripts behave exactly as expected across prod/dev/all contours.
...
- **Smoke test A: full installer/uninstaller matrix + cleanup (`tests/...installer...smoke...`):**
  - Cover every script entrypoint (mandatory matrix):
    - centralized: `install-service.sh`, `uninstall-service.sh`
    - wrappers: `install-service-prod.sh`, `install-service-dev.sh`, `uninstall-service-prod.sh`, `uninstall-service-dev.sh`
    - service-specific: `engine/install-engine-service.sh`, `client/install-client-service.sh`, `engine/uninstall-engine-service.sh`, `client/uninstall-client-service.sh`
  - For each entrypoint, run contract checks:
    - `--help` returns success and includes key mode/flag contract,
    - `--dry-run` returns success and prints expected contour unit names/actions.
  - Run functional install->verify->uninstall->verify checks for both contours:
    - `prod`: verify Engine/Client units active after install and absent/inactive after uninstall,
    - `dev`: same verification with dev unit names and dev ports.
  - Validate `--mode all` behavior for centralized install/uninstall scripts.
  - Validate updater artifacts behavior:
    - timer/sudoers removal on uninstall,
    - optional updater-state purge behavior when enabled in test flags.
  - Validate idempotency:
    - repeated install on same contour must succeed,
    - repeated uninstall on same contour must succeed without residual failures.
  - Validate contour isolation:
    - prod operations do not modify dev units,
    - dev operations do not modify prod units.
  ...
  - Always perform teardown in `finally/trap` logic:
    - invoke centralized/service-specific uninstall scripts for dev contour as source of truth,
    - stop/disable/remove any leftover dev units created by the test (safety fallback),
    - remove test-created timer/unit/state artifacts when applicable,
...
- **Expected runtime behavior (mandatory acceptance):**
  - Engine and Client services must not only be `active`; both servers must respond over HTTP after install.
  - Prod contour must run on its own service names/ports and dev contour must run on its own service names/ports with no port overlap.
  - Client routing must be contour-local only:
    - prod Client -> prod Engine,
    - dev Client -> dev Engine.
  - Engine health/read endpoints must return expected `200` responses with valid response shape.
  - Client health/write endpoints must be operational and must not fail due to wrong Engine routing.
  - Minimal e2e must prove write path reaches Engine ingest:
    - send action via Client API (`/api/user-action`),
    - verify expected successful bridge/ingest result.
  - After uninstall, servers must stop responding over HTTP and contour unit/timer/sudoers artifacts must be removed as expected.
  - Repeated install/uninstall runs must remain stable (idempotent) without accumulating residual state.
- **Isolation and safety rules:**
  ...
  - Cleanup path must use uninstall scripts first, then fallback direct cleanup only for residuals.
...
- **Validation / CI expectations:**
  ...
  - Final acceptance criterion: all installer/uninstaller matrix checks pass with zero failed assertions, and no residual test services/artifacts remain.

#### **Affected areas/files (expected):**
- New test directory and scripts: `tests/*` (installer/uninstaller smoke + interaction smoke + optional runner/shared utils).
- Service scripts touched by tests: `install-service.sh`, `install-service-dev.sh`, `install-service-prod.sh`, `engine/install-engine-service.sh`, `client/install-client-service.sh`, `uninstall-service.sh`, `uninstall-service-dev.sh`, `uninstall-service-prod.sh`, `engine/uninstall-engine-service.sh`, `client/uninstall-client-service.sh` (invoked, not necessarily modified).
