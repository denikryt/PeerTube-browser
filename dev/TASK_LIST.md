...
### 55) Strict error on too many likes in recommendations request
**Problem:** Engine currently truncates extra likes silently when request body includes more likes than allowed, which hides API contract violations and makes debugging harder.

**Solution option:** reject oversized likes payloads with explicit `400` instead of silently cutting the list.

#### **Solution details:**
- Validate client `likes` count before like normalization/resolution in the recommendations request path.
- If `likes` count exceeds `DEFAULT_CLIENT_LIKES_MAX`, return `400` with clear machine-readable details (`error`, `max_allowed`, `received`).
- Keep existing item-level validation (uuid/host shape) for requests within the allowed count.
- Keep body-size limit behavior unchanged.
- Validation:
  - request with `likes` count `> DEFAULT_CLIENT_LIKES_MAX` returns `400`,
  - request with `likes` count `<= DEFAULT_CLIENT_LIKES_MAX` preserves current success flow.
