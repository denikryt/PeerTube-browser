...
### 57) Split Engine/Client into subtree remotes + fast sync push script
**Problem:** engine and client should stay in one local development workspace, but must be pushed as separate remote repositories with minimal manual git steps.

**Solution option:** formalize a `git subtree` workflow and add one sync script that splits and pushes both prefixes.

#### **Solution details:**
- Add subtree remote contract:
  - `engine` subtree source prefix: `engine/`,
  - `client` subtree source prefix: `client/`,
  - configurable remote names and target branches for each subtree repo.
- One-time bootstrap is manual (done by user), not auto-created by script:
  - provide explicit step-by-step commands to add remotes and create initial target branches in both subtree repos;
  - script assumes remotes/branches already exist and fails with clear help text if they are missing.
- Add one operational script (for example `sync-subtrees.sh`) that:
  - validates current branch and optional clean-worktree requirement,
  - runs `git subtree split --prefix=engine` and `git push` to engine remote branch,
  - runs `git subtree split --prefix=client` and `git push` to client remote branch,
  - supports partial sync (`--engine-only` / `--client-only`) and `--dry-run`.
- Keep a deterministic push strategy:
  - always split from current checked-out branch,
  - avoid merge commits in subtree remotes (linear split history).
- Add concise runbook docs:
  - first-time manual bootstrap (`git remote add`, initial split push, branch creation by user),
  - regular update flow (single sync script command),
  - hotfix-backport flow (`git subtree pull` from subtree remote when needed).
- Validation / tests:
  - dry-run prints exact git commands without pushing;
  - real run updates both remotes with expected subtree-only history;
  - repeated run with no source changes produces no-op/unchanged remote heads.
