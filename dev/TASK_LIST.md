...
### 52) Route all Engine read requests through Client backend (UI must not call Engine directly)
**Problem:** frontend currently uses Engine API directly for recommendations/similar/read metadata, which keeps UI coupled to Engine topology and bypasses Client as the single request gateway.

**Solution option:** make Client backend the only public API surface for frontend reads and writes; Client should proxy required Engine read endpoints internally.

#### **Solution details:**
- Add/extend Client read-proxy endpoints for Engine-backed reads (recommendations/similar/video-read flows used by UI).
- Keep Engine read endpoints internal from UI perspective; UI should use Client API base only.
- Update frontend API base usage to remove direct Engine read calls in browser code.
- Ensure Client->Engine forwarding contract has explicit timeout/retry/error mapping and deterministic response shape for UI.
- Add strict allowlist validation for forwarded query/body parameters to avoid open-proxy behavior.
- Keep contour-local routing invariant:
  - prod UI -> prod Client -> prod Engine,
  - dev UI -> dev Client -> dev Engine.
- Update/install docs to reflect that frontend runtime needs only Client API base.
- Validation / tests:
  - smoke/e2e: recommendations/similar/video-read requests succeed through Client proxy path;
  - boundary test: frontend code does not call Engine API base directly;
  - regression: write/profile endpoints remain operational with the same Client base.
