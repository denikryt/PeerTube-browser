...
### 66) [M1][F1] Freeze Engine/Client ownership contract in canonical docs
**Problem:** boundary ownership wording is still spread across docs and can be interpreted differently, which risks reintroducing Engine/Client coupling.

**Solution option:** consolidate one explicit boundary contract in canonical docs and remove contradictory wording.

#### **Solution details:**
- Define one source-of-truth boundary contract that explicitly states:
  - Engine public read/API ownership,
  - Client browser-facing ownership,
  - internal Client -> Engine read contract,
  - temporary bridge ingest contract.
- Normalize wording in canonical docs so all boundary descriptions match this contract.
- Keep scope limited to boundary contract freeze for already implemented behavior (no new feature scope).

#### **Concrete steps:**
1. Audit current boundary wording in `README.md` and service READMEs and list mismatches.
2. Update canonical docs with one explicit ownership matrix for Engine/Client endpoint responsibilities.
3. Remove or rewrite conflicting wording so only one contract interpretation remains.
4. Verify consistency by scanning updated docs for boundary keywords and cross-checking statements.

### 67) [M1][F1] Harden boundary checks for forbidden coupling and direct frontend Engine reads
**Problem:** existing boundary checks may miss new coupling regressions (forbidden imports, DB coupling, or direct frontend reads to Engine base).

**Solution option:** strengthen automated boundary guards and keep them integrated into split smoke flow.

#### **Solution details:**
- Extend boundary checks for:
  - forbidden Client imports from Engine internals,
  - forbidden direct Client DB coupling into Engine data paths,
  - forbidden direct frontend runtime reads to Engine API base.
- Keep checks runnable in one repeatable flow used by split smoke validation.

#### **Concrete steps:**
1. Review `tests/check-client-engine-boundary.sh` rules and add missing forbidden-coupling patterns.
2. Ensure split smoke entrypoint (`tests/run-arch-split-smoke.sh`) executes the updated boundary check path.
3. Run both scripts locally and confirm violations are reported with actionable error messages.
4. Document expected failure classes covered by the updated checks.

### 68) [M1][F1] Validate local/dev split contour and document failure semantics
**Problem:** split contour validation expectations are not captured as one reproducible baseline run with explicit pass/fail semantics.

**Solution option:** run the local/dev split validation path end-to-end and document exact expected outcomes.

#### **Solution details:**
- Validate startup and health for Engine + Client split services.
- Validate bridge/read flow and boundary-check path in the same run.
- Record pass/fail semantics and troubleshooting expectations for reproducible Milestone 1 baseline approval.

#### **Concrete steps:**
1. Execute split validation flow (`tests/run-arch-split-smoke.sh`) in local/dev configuration.
2. Confirm health, boundary, and bridge stages complete without ownership regressions.
3. Capture expected pass/fail semantics for each stage in canonical docs/runbook notes.
4. Re-run validation once after doc updates to confirm instructions and checks stay aligned.
