...
### 59) [M1][SI1] Materialize DEV_MAP milestones and features to GitHub
**Problem:** DEV_MAP already defines milestones/features, but their high-level roadmap structure is not synchronized with GitHub milestones/issues.

**Solution option:** materialize DEV_MAP roadmap entities in GitHub using only existing DEV_MAP texts (no rewritten descriptions): create/sync milestones and create one high-level issue per feature.

#### **Solution details:**
- Create/resolve GitHub milestones for all current DEV_MAP milestones (`M1..M8`) and fill milestone descriptions strictly from DEV_MAP fields (`title`, `goal`, related feature titles/descriptions) without generating new wording.
- Create one high-level GitHub issue per DEV_MAP feature using feature text from DEV_MAP as-is (title/description source), without assigning GitHub milestones to issues.
- Persist links back into DEV_MAP for synchronized feature issues (`gh_issue_number`, `gh_issue_url`).
- Keep this task high-level only: do not create task-level GitHub issues and do not alter feature/task statuses.
- Validation:
  - each DEV_MAP milestone has a matching GitHub milestone;
  - each feature has a linked high-level GitHub issue created from DEV_MAP text;
  - no GitHub milestone is set on feature issues;
  - DEV_MAP stores resulting GitHub issue references for synchronized features.
