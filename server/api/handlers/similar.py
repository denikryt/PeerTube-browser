"""Similarity and profile HTTP handler.

Routes:
- /api/similar (GET/POST): recommendation feed and debug payloads.
- /api/health: health check.
- /api/channels: channels listing.
- /api/video: single video metadata.
- /api/user-action: record like/dislike.
- /api/user-profile (+ /likes, /reset): user likes/profile endpoints.
...
from handlers.user_profile import (
    handle_user_action,
    handle_user_profile,
    handle_user_profile_likes,
    handle_user_profile_likes_from_client,
    handle_user_profile_reset,
)
...
class SimilarHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        """Handle user actions and profile reset."""
        ...
        if url.path == "/api/user-action":
            if not self._rate_limit_check(url.path):
                ...
                return
            handle_user_action(self, self.server)
            return
        if url.path == "/api/user-profile/likes":
            if not self._rate_limit_check(url.path):
                respond_json(self, 429, {"error": "Rate limit exceeded"})
                return
            handle_user_profile_likes_from_client(self, self.server)
            return
        if url.path == "/api/user-profile/reset":
            if not self._rate_limit_check(url.path):
                respond_json(self, 429, {"error": "Rate limit exceeded"})
                return
            handle_user_profile_reset(self, self.server)
            return
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if url.path == "/api/user-profile":
            handle_user_profile(self, self.server, params)
            return

        if url.path == "/api/user-profile/likes":
            handle_user_profile_likes(self, self.server, params)
            return
        ...
        self._handle_similar_request(method="GET")
