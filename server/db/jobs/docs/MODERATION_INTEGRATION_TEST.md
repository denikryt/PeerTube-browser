...
`server/db/jobs/tests/test-moderation-integration.py`.
...
```bash
python3 server/db/jobs/tests/test-moderation-integration.py
```

Run with sampled production data:

```bash
python3 server/db/jobs/tests/test-moderation-integration.py --sample-from-prod
```

Explicit source DB:

```bash
python3 server/db/jobs/tests/test-moderation-integration.py --sample-from-prod --source-db server/db/whitelist.db
```

Run with a specific host as block/purge target:

```bash
python3 server/db/jobs/tests/test-moderation-integration.py --sample-from-prod --target-host bad.instance.tld
```

Keep fixture files for inspection:

```bash
python3 server/db/jobs/tests/test-moderation-integration.py --keep-workdir
```
...
- The test validates serving moderation logic through the shared serving helper used by `/api/similar`.
- It does not perform real HTTP requests to `/api/similar`; it verifies the same filtering contract in-process.
