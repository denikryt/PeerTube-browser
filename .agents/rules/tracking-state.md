...
4. `clean feature <id>` and `clean issue <id>` are the canonical local artifact cleanup actions.
5. `clean` must not perform remote close or remote issue mutation.
6. Keep `dev/FEATURE_PLANS.md`, `dev/map/DEV_MAP.json`, and runtime command semantics aligned when lifecycle rules change.
