...
5. Read `Expected Behaviour` from the relevant feature and issue plan blocks in `dev/FEATURE_PLANS.md` and keep task scopes aligned with that declared runtime outcome.
6. Ensure task marker correctness: Formulate `[M*][F*]` or `[M*][SI*]` strings for tasks based on their parent chain.
7. Quality Check: Verify compliance with **Gate B: Pre-sync (local decomposition)** in `.agents/protocols/feature-planning-protocol.md`.
8. Only after task decomposition is completely mapped out conceptually from code evidence and quality check is passed, utilize the CLI to synchronize everything.

// turbo
9. Run: `python3 dev/workflow plan tasks --feature-id <feature_id>` OR `python3 dev/workflow plan tasks --issue-id <issue_id> ...`

10. Review the synced tasks in `dev/TASK_LIST.json` and `dev/map/DEV_MAP.json` to ensure `concrete_steps`, markers, and issue/task ownership reflect the actual code-informed decomposition.
11. Stop and use `notify_user` with `BlockedOnUser=true` to await explicitly synced local decomposition review.
