.PHONY: test test-fast test-python test-boundaries test-python-compile test-legacy-interaction-events test-jobs test-framework build-frontend build-crawler test-crawler-db test-frontend test-smoke-arch test-installers-dry-run lint
...
test-jobs:
	python3 -m pytest tests/jobs tests/framework -q
