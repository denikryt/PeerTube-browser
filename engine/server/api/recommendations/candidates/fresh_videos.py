"""Provide fresh videos runtime helpers."""
...
class FreshVideosDeps:
    """Represent fresh videos deps behavior."""
...
class FreshVideosGenerator:
    """Represent fresh videos generator behavior."""
    name = "fresh"

    def __init__(self, deps: FreshVideosDeps) -> None:
    ...
    def get_candidates(
        ...
        if config:
            try:
                config_pool_size = int(config.get("pool_size") or 0)
            except (TypeError, ValueError):
                config_pool_size = 0
            max_per_instance = int(config.get("max_per_instance") or 0)
            max_per_author = int(config.get("max_per_author") or 0)
        pool_limit = max(config_pool_size or int(self.deps.pool_size or 0), limit)
        pool_start = perf_counter()
        with server.db_lock:
            pool = self.deps.fetch_recent_videos(server.db, pool_limit)
        ...
        if not likes:
            ...
            logging.info(
                "[recommendations] fresh timing pool=%dms total=%dms size=%d",
                pool_ms,
                total_ms,
                len(pool),
            )
        ...
        if not liked_embeddings or not pool_embeddings:
            total_ms = int((perf_counter() - pool_start) * 1000)
            logging.info(
                "[recommendations] fresh timing pool=%dms emb=%dms total=%dms size=%d",
                pool_ms,
                emb_ms,
                total_ms,
                len(pool),
        ...
        total_ms = int((perf_counter() - pool_start) * 1000)
        logging.info(
            "[recommendations] fresh timing pool=%dms emb=%dms score=%dms total=%dms size=%d",
            pool_ms,
            emb_ms,
            score_ms,
            total_ms,
...
def _random_from_pool(pool: list[dict[str, Any]], limit: int) -> list[dict[str, Any]]:
