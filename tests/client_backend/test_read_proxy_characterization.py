...
def test_get_api_video_rejects_unknown_query_parameter(start_json_engine, start_client_backend, http_json) -> None:
    """Unknown read query parameters must not become accidental public API surface."""
    fake_engine = start_json_engine(
        {("GET", "/api/video"): lambda _record: (200, {"video": {"video_id": "123", "title": "Example"}})}
    )
    client = start_client_backend(f"http://127.0.0.1:{fake_engine.server_port}")

    status, body = http_json(
        "GET",
        f"http://127.0.0.1:{client.server_port}/api/video?id=123&host=example.org&unknown=x",
    )
...
def test_get_api_video_forwards_allowlisted_query_parameters(start_json_engine, start_client_backend, http_json) -> None:
...
def test_post_recommendations_forwards_sanitized_body(start_json_engine, start_client_backend, http_json) -> None:
    """Recommendations proxy must forward only current allowed keys and sanitized likes."""
    fake_engine = start_json_engine(
        {("POST", "/recommendations"): lambda _record: (200, {"generatedAt": 1, "total": 0, "count": 0, "seed": None, "rows": []})}
    )
    client = start_client_backend(f"http://127.0.0.1:{fake_engine.server_port}")

    status, body = http_json(
        "POST",
        f"http://127.0.0.1:{client.server_port}/recommendations",
...
def test_post_recommendations_rejects_unknown_body_field(start_json_engine, start_client_backend, http_json) -> None:
